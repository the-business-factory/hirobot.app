FROM robcole/crystal:1.4.1-alpine-hoard as crystal_dependencies
...
FROM robcole/crystal:1.4.1-alpine-hoard as lucky_tasks_build
ENV LUCKY_ENV=production
...
FROM robcole/crystal:1.4.1-alpine-hoard as lucky_webserver_build
...
COPY --from=asset_build /assets/public public
COPY --from=robcole/crystal:1.4.1-alpine-hoard /tmp/Hoard /app/Hoard
